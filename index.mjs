...
const getCoinQuote = (inputMint, outputMint, amount) =>
    got
        .get(
            `https://quote-api.jup.ag/v1/quote?outputMint=${outputMint}&inputMint=${inputMint}&amount=${amount}&slippage=0.5`
        )
        .json();
...
const initial = 3_000_000_000;
const initialDecimal = 3;
...
while (true) {
    ...
    transactionProfit = 0;
    // 0.1 SOL
    console.log(`Iteration #: ${iterationsTotal} | Checking quotes using ${initial} sol as input amount...`);
    const solToSol = await getCoinQuote(SOL_MINT, SOL_MINT, initial);
    const outAmount = solToSol.data[0].outAmount;
    // when outAmount more than initial
    if (outAmount > initial) {
        transactionsAttempted++;
        await Promise.all(
            [solToSol.data[0]].map(async (route) => {
        ...
        transactionProfit = (solToSol.data[0].outAmount / 1000000000) - initialDecimal;
        totalProfit += transactionProfit;
    }
    console.log( `-Quote information: SOL_TO_SOL ->  ${outAmount} | OUT_AMOUNT_W_SLIPPAGE -> ${solToSol.data[0].outAmountWithSlippage} | OUT_AMOUNT > INITIAL: ${outAmount > initial} `);
    console.log(
